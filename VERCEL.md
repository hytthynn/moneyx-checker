...
Для базы, созданной до появления настройки порога роста, дополнительно выполните `migrations/002_increase_threshold.sql` перед развёртыванием новой версии бота.
...
Добавьте бота в группу из `TELEGRAM_GROUP_ID` администратором с правами отправлять и удалять сообщения. Команды выполняйте только в личном чате с ботом.

Настройте авторизацию Money-X одним из способов:

- в личном чате отправьте `/start`, нажмите «Изменить авторизацию» и следуйте подсказкам;
- либо локально подключитесь к той же Supabase базе с тем же `APP_ENCRYPTION_KEY` и выполните `python -m scripts.set_auth`.

После этого откройте `/start`, проверьте «Статус» и нажмите «Проверить курс». Отчёт должен появиться только в группе.
...
2. «Статус» в панели `/start` показывает состояние авторизации Money-X.
3. Кнопка «Проверить курс» отправляет полный отчёт в нужную группу.
4. Сообщения пользователей, включая команды, в группе автоматически удаляются.
