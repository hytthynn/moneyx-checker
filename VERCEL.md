...
По возможности выбирайте близкие регионы Supabase и Vercel: это уменьшает задержку каждого обращения к базе.
...
- повтор при сетевой ошибке: не более одного, поскольку приложение само защищено от дублей.

Сначала используйте кнопку ручного запуска задачи. Успешный ответ имеет `status` со значением `sent` или `duplicate`. Ответ `401` означает неверный заголовок, `failed` — ошибку Money-X или Telegram, а `busy` — уже выполняющуюся проверку.
...
5. Ручной запуск cronjob возвращает `sent` и не создаёт дубль при повторном вызове.
